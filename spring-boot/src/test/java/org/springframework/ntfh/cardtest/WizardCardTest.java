...
public class WizardCardTest {
	...
	void testAuraProtectora(){
		...
		wizard.setHand(hand);
		abilityCardIngameService.playCard(abilityCardIngameWizard.getId(),
				null, tokenWizard);
	...
	void testBolaDeFuego(){
        ...
        AbilityCardIngame abilityCardIngameWizard =
        abilityCardIngameService.createFromAbilityCard(bolaDeFuego, wizard);
        String tokenWizard = TokenUtils.generateJWTToken(wizard.getUser());
		List<AbilityCardIngame> hand = new ArrayList<>();
		hand.add(abilityCardIngameWizard);
		wizard.setHand(hand);
	...
	void testDisparoGelido(){
		...
		wizard.setHand(hand);
		abilityCardIngameService.playCard(abilityCardIngameWizard.getId(),
				berserkerIngame.getId(), tokenWizard);
	...
	void testFlechaCorrosiva(){
		...
		AbilityCard disparoGelido = abilityCardService.findById(33);
        AbilityCardIngame disparoGelidoIngame =
        abilityCardIngameService.createFromAbilityCard(disparoGelido, wizard);

		hand.add(disparoGelidoIngame);
		wizard.setHand(hand);
		abilityCardIngameService.playCard(disparoGelidoIngame.getId(),
				berserkerIngame.getId(), tokenWizard);
	...
	void testOrbeCurativo(){
		...
		assertThat(wizard.getDiscardPile().size()).isZero(); //the card played has the exile property, thus it didnt go to the discard pile
	}
	...
	void testReconstitucion(){
		...
		assertThat(wizard.getDiscardPile().size()).isEqualTo(1); //the card played is discarded but the other 2 have been recovered
	}
