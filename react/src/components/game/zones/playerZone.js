...
export default function PlayerZone(params) {
  ...
  const rotation = params.rotation ?? 0; // For those on the sides
  const cards = [1, 2, 3]; // Placeholder
  const sides = rotation !== 0;
  useEffect(() => {
  ...
  return (
    <div
      className={`flex flex-${sides ? "col" : "row"}${
        reverse ? "-reverse" : ""
      } space-${sides ? "y" : "x"}-4 items-end`}
    >
      <div
        className={`flex flex-${
          sides ? "row" : "col"
        } space-y-2 mx-2 h-full justify-end items-baseline`}
      >
